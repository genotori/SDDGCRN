...
class Trainer(object):
    def __init__(self, model, loss, optimizer, train_loader, val_loader, test_loader, scaler, args, lr_scheduler=None):
        ...
        self.best_test_path = os.path.join(self.args.log_dir, args.dataset + '.pth')
        self.loss_figure_path = os.path.join(self.args.log_dir, 'loss.png')
        self.adj = load_adj(args.dataset, os.path.join('./data/' + args.dataset + '/' + args.dataset + '.csv'), args.num_nodes, k=args.normalizer_k, sym_graph=True, zero_one=False)
        # log
    ...
    def val_epoch(self, epoch, val_dataloader, i=2):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                output = self.model(data, self.adj, epoch)
    ...
    def train(self):
        ...
        if not self.args.debug:
            ...
            self.logger.info(
                "Saving current best model to " + self.best_path + ', Which best epoch is ' + str(best_epoch))
            torch.save(best_test_model, self.best_test_path)
            self.logger.info(
                "Saving current best model to " + self.best_test_path + ', Which best test epoch is ' + str(
                    best_test_epoch))

        # test
        self.logger.info("This is best_val_model")
        self.model.load_state_dict(best_model)
        # self.val_epoch(self.args.epochs, self.test_loader)
        self.test(self.model, self.args, self.val_loader, self.scaler, self.logger, adj=self.adj, best_epoch=best_epoch)

        self.logger.info("This is best_test_model")
        self.model.load_state_dict(best_test_model)
        self.test(self.model, self.args, self.test_loader, self.scaler, self.logger, adj=self.adj,
    ...
    @staticmethod
    def test(model, args, data_loader, scaler, logger, path=None, adj=None, best_epoch=1):
        if path != None:
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(data_loader):
                ...
                output = model(data, adj, best_epoch)
                y_true.append(label)
